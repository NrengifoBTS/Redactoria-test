...
class ContentGenerator:
    ...
    def generate_fleet(self, tit_seo: str, nuevo_tema: str) -> str:
        ...
        location = self._extract_location_from_title(nuevo_tema)
        tit = f"Variedad de autos en {location}"
        ...
        if _BENEFICIOS_LATAM not in desc:
            desc = (
                f"{desc.strip()} Incluye: {_BENEFICIOS_LATAM} y mucho más."
            )
    ...
    def generate_agencies(self, tit_seo: str, nuevo_tema: str) -> str:
        ...
        if self.brand == "mcr":
            ...
            rules = (
                ...
                f"- desc_h2: 50 a 65 palabras. OBLIGATORIO: posicionar {brand_name} como plataforma con más de 15 años que conecta al viajero "
            ...
            rules = (
                ...
                "'Compañías de renta de carros en X' / 'Agencias líderes de alquiler de autos en X'.\n"
                f"- desc_h2: 50 a 65 palabras. OBLIGATORIO: posicionar {brand_name} como plataforma que conecta al viajero con múltiples agencias "
    ...
    _CAR_RENTAL_TITULOS = [
        "Elige el auto ideal para tu viaje por {loc}",
        "Categorías de autos disponibles en {loc}",
        "Encuentra el vehículo que mejor se adapta a tu ruta en {loc}",
        "Tipos de autos para recorrer {loc} a tu ritmo",
        "El auto correcto para cada tipo de viaje en {loc}",
        "Opciones de vehículos para tu estancia en {loc}",
    ]
    ...
    def generate_car_rental(self, tit_seo: str, nuevo_tema: str) -> str:
        ...
        prompt = (
            ...
            "REGLAS:\n"
            f"- Tono: {tone_rule}\n"
            "- tit: título H2 concreto, sin fórmulas genéricas. Máximo 10 palabras.\n"
            f"{desc_rule}"
            "Genera:\n"
            "<think>aquí tus pensamientos</think>\n"
            f"|tit: {tit_sugerido}|\n"
    ...
    def generate_rentacar(self, tit_seo: str, nuevo_tema: str) -> str:
        ...
        # ── Llamada 1: tit + desc (introducción H2, 70-90p) ──
        p1 = (
            f"Nuevo tema: {nuevo_tema}, contexto LP: {tit_seo}\n"
            ...
            "- desc: 70 a 90 palabras. Introducción sobre el destino y por qué vale la pena "
        ...
        if not tit_2:
            tit_2 = f"Agencias de autos disponibles en {location}"
        if not desc_2:
    ...
    def generate_bloquehistoria(self, tit_seo: str, nuevo_tema: str) -> str:
        ...
        prompt = (
            ...
            "REGLAS:\n"
            "- tit: H2 breve, natural y específico del destino.\n"
